...
crdc_join_atl <-
  crdc_join |>
  filter(str_detect(as.character(COMBOKEY), "^1300026"))
...
atl_pct_join <-
  left_join(atl_tot_enr_calc, atl_tot_rst_calc, by = c("RACE_RST_CHR", "SEX_RST")) |>
...
atl_pct_calc |>  
  ...
  coord_flip() +
  theme_minimal() +
  labs(title = paste("Percent of Students Mechanically Restrained", get_county_name(13121), "2021-2022"),
       x = "Race",
...
crdc_enroll_cleaned <- inner_join(crdc_enroll_third_clean, crdc_enroll_total, by = "COMBOKEY") |>
  mutate(PCT_ENR_GEN_RACE = (ENROLLMENT/TOTAL_ENR)*100) |>
  filter(TYPE == "School")
...
crdc_race_gender_match <- crdc_join_onlyyes |>
